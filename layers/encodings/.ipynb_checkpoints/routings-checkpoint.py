...
#     ''' n to 1 '''
#     def __init__(self, config, input_dim, C=1, R=1, n_iter=3):
#         super().__init__()
#         self.config = config
#         self.n_iter = n_iter
...
#             self.C, 1, self.R, self.input_dim, self.output_dim,
#         ))
#         bias = np.sqrt(6.0 / (self.input_dim + self.output_dim))
#         nn.init.uniform_(self.W.data, -bias, bias)
        
#     def forward(self, inputs):
#         B, R, input_dim = inputs.shape
#         C, output_dim = self.C, self.output_dim
        
#         u = inputs[None, :, :, None, :] @ self.W # (C, B, R, 1, out)
        
#         q = inputs.mean(-2, keepdims=True) # (..., 1, H)
#         k = v = inputs                     # (..., n, H)
        
#         for i in range(self.n_iter):
#             w = ((k @ q.transpose(-1, -2)) / np.sqrt(k.shape[-1])).softmax(-2) # (..., n, 1)
#             q = (w * v).sum(-2, keepdims=True) # (..., n, 1) * (..., n, H) => (..., 1, H)
